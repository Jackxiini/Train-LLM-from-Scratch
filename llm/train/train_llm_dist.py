...
def train_LLM(
    ...
    """Train a language model using distributed training."""
    is_main_process = rank == 0
    
    # Handle output directory creation (only on main process)
    if is_main_process:
    ...
    if args.resume_from_checkpoint is not None:
        ...
        start_iter = load_checkpoint(model.module if isinstance(model, DDP) else model, 
                                   optimizer, args.resume_from_checkpoint)
        if is_main_process:
    ...
    for step, batch in enumerate(tqdm(train_dataloader, total=num_iters, initial=start_iter, 
        ...
        if world_size > 1:
            ...
            dist.all_reduce(perplexity, op=dist.ReduceOp.AVG)
        ...
        if is_main_process:
            ...
            if step % checkpoint_interval == 0:
                ...
                save_checkpoint(model.module if isinstance(model, DDP) else model, 
                              optimizer, step, checkpoint_path)
            
            if step > 0 and (step % args.validation_interval == 0 or step == num_iters - 1):
                logger.info(f"Running validation at step {step}")
                avg_loss, avg_perplexity = validate(model.module if isinstance(model, DDP) else model, 
                                                  val_dataloader, device, precision)
                ...
                if avg_loss < best_loss:
                    ...
                    save_checkpoint(model.module if isinstance(model, DDP) else model, 
                                  optimizer, step, best_model_path)
                
                logger.info("Generating sample text...")
                generated_text = run_generation(model.module if isinstance(model, DDP) else model, 
                                             tokenizer, args.val_prompt, 
    ...
    if is_main_process:
        ...
        save_checkpoint(model.module if isinstance(model, DDP) else model, 
                       optimizer, args.num_iters, final_model_path)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run distributed LLM training with Transformer architecture",
    ...
    parser.add_argument("--special_tokens", type=list[str], default=["<|endoftext|>"],
                      help="List of special tokens to add to vocabulary")
    ...
    parser.add_argument("--val-prompt", type=str, default=VAL_PROMPTS[0],
    ...
    parser.add_argument("--dist_port", type=str, default="29500",
    ...
    if rank == 0:
        logger.info(f"Set random seed to {args.seed} (rank {rank})")
    ...
    train_dataloader = random_training_iterator(
        train_dataset, 
        args.batch_size // world_size,  # Adjust batch size per GPU
        args.context_length, 
        device, 
        args.num_iters
    )
