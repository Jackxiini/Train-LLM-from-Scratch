...
from llm.layers import (
    ...
    softmax, scaled_dot_product_attention, CausalMultiHeadAttention, CausalMultiHeadAttentionRoPE)
from llm.transformer import TransformerBlock, TransformerLM
from llm.nn_utils import cross_entropy
from llm.optimizer import AdamW, SGD, get_lr_cosine_schedule, gradient_clipping
...
def run_multihead_self_attention(
    ...
    """
    causal_MHA = CausalMultiHeadAttention(d_model, num_heads, device=q_proj_weight.device, dtype=q_proj_weight.dtype)
    causal_MHA.load_state_dict({
        "qkv_proj.weight": torch.cat([q_proj_weight, k_proj_weight, v_proj_weight], dim=0),
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    causal_MHARoPE = CausalMultiHeadAttentionRoPE(
        d_model, num_heads, max_seq_len, theta, device=q_proj_weight.device, dtype=q_proj_weight.dtype)
    causal_MHARoPE.load_state_dict({
        "qkv_proj.weight": torch.cat([q_proj_weight, k_proj_weight, v_proj_weight], dim=0),
        "output_proj.weight": o_proj_weight,
    })
    return causal_MHARoPE(in_features, token_positions)
...
def run_transformer_block(
    ...
    # 把 attn.q_proj/k_proj/v_proj 三个拼在一起
    sd["attn.qkv_proj.weight"] = torch.cat([
        weights["attn.q_proj.weight"],
        weights["attn.k_proj.weight"],
        weights["attn.v_proj.weight"],
    ], dim=0)
    # 其余直接简单映射
    sd["attn.output_proj.weight"]  = weights["attn.output_proj.weight"]
    ...
    # 3) 生成 token_positions 并传入
    B, L, _ = in_features.shape
    token_positions = (
        torch.arange(L, device=in_features.device, dtype=torch.long)
             .unsqueeze(0)       # (1, L)
             .expand(B, -1)       # (B, L)
    )
...
def run_transformer_lm(
    ...
    # 3) 重命名 "layers." -> "transformer."，构造真正要加载给 load_state_dict 的字典
    state_dict: dict[str, Tensor] = {}
    for key, val in weights.items():
        if key.startswith("layers."):
            # 把 "layers.2.ln1.weight" -> "transformer.2.ln1.weight"
            state_dict[key.replace("layers", "transformer", 1)] = val
        else:
            # token_embeddings.weight, ln_final.weight, lm_head.weight 保留不变
            state_dict[key] = val
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    # Call the function from your bpe.py
    # The train_bpe_from_files expects a list of input_paths.
    # If input_path is a single file, wrap it in a list.
    return run_train_bpe(
        input_path=[input_path] if isinstance(input_path, str) else input_path,
        vocab_size=vocab_size,
        special_tokens=special_tokens[0] if special_tokens else None,
        **kwargs,
    )
